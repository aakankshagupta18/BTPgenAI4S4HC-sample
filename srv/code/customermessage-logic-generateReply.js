/**
 * 
 * @Before(event = { "Action1" }, entity = "aakankshagupta_ds1_a29Srv.CustomerMessage")
...
try {
    ...
    const customerMessage = await SELECT.one.from('aakankshagupta_ds1_a29.CustomerMessage').where({ ID });
    ...
    if (messageCategory === 'Technical') {
        ...
        const relevantFAQs = await SELECT`id, issue, question, answer`
            .from('aakankshagupta_ds1_a29.ProductFAQ')
    ...
    await UPDATE('aakankshagupta_ds1_a29.CustomerMessage').set({
        suggestedResponseCustomerLanguage,
        suggestedResponseEnglish,
    }).where({ ID });
