...
module.exports = async function(results, request) {
	...
	try {
		...
		let productFAQ = await SELECT.one('aakankshagupta_ds1_a29.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		...
		await UPDATE("aakankshagupta_ds1_a29.ProductFAQ", ID).with({ embedding: utils.array2VectorBuffer(embedding) });
